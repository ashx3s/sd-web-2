...
#### A note on how this changes in app development

_React, Vue, Angular, Svelte etc all have other ways of dealing with the doctype and head type details. So while it's good to refresh on this. You won't be writing html like this in this semester past assignment 1_
...
- Avoid using var. This is globally scoped and can lead to unintended side effects.
- Instead, make a clear code functionality decision with regards to scope and mutability between let and const

#### Variable Declaration Types
- `let`: mutable and good for when a variable content needs to change. Block scoped
- `const`: non-mutable (but you can edit the contents of arrays and objects). also block scoped. It's generally a good practice to default to `const`
...
- **Array**: Similar to a list in python. Arrays are iterable and you'll use these extensively within Javascript app development
...
Array methods like `.map()`, `.forEach()`, `.filter()`, `.reduce()` are fantastic for working with data that is fetched from a backend or api. React particularly uses `.map()` for rendering iterable data
...
Arrow notation is an es6 convention for function declaration as well. This pattern has seen considerable adoption in frontend javascript development. It is different from `function` declarations in that it does not inherit `this`. **We will cover use cases as we dive into react development.
