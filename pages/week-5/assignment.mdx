...
In this assignment, we will combine the components we built in Weeks 4 to create a fully functional "Shopping List" application.
...
- Create a new `page.js` file inside `/app/week-5`.
...
- Create a _sorted_ version of the `items` prop based on the `sortBy` state.
- _Note:_ Do not mutate the original prop. Use `[...items].sort(...)`.
