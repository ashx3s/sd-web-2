...
Create the folder `/app/week-3` and create files `GroceryItem.js`, `GroceryItemList.js`, and `page.js` inside it.
...
In `GroceryItemList.js`, create a functional component named `GroceryItemList`. Inside this component, copy paste the following item objects each with `name`, `quantity`, and `category` properties. Render these items using the `Item` component you just created, passing item data as props.
...
https://cprg306-assignments.vercel.app/week-3
...
- Submit your repo and deployed site links to brightspace
